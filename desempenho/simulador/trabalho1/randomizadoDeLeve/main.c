...
int main(void){
    ...
    //printf("[DEBUG] primeira chegada agendada em t=%.6lf\n", proxima_requisicao);
    ...
    while(tempo_decorrido < tempo_simulacao){
        ...
        if(fabs(tempo_decorrido - proxima_requisicao) < 1e-9){
            ...
            //printf("[DEBUG] proxima chegada agendada em t=%.6lf\n", proxima_requisicao);
            ...
            // Saída
            fila--;
            //printf("[DEBUG] saida em t=%.6lf, fila=%lu\n", tempo_decorrido, fila);

            if(fila){
                tempo_servico = tempo_decorrido + exponencial(media_tempo_servico);
                qtd_servicos++;
                soma_tempo_servico += tempo_servico - tempo_decorrido;
                //printf("[DEBUG] novo servico iniciado, termino previsto em t=%.6lf\n", tempo_servico);
            }
