...
"""
Queueing Simulation Performance Analysis Toolkit
Author: Rafael Passos Domingues
Last Update: Octuber 23, 2025

Analysis of multi-queue scheduling policies with:
...
class AnalysisConfiguration:
    ...
    def __init__(self):
        ...
        # Column Name Mapping (aligned with C simulator output)
...
class SimulationAnalysisPipeline:
    ...
    def _fitAnalyticalModels(self, summaryData: pd.DataFrame) -> Dict[str, Dict[str, Any]]:
        ...
        # Model E[N] vs occupancy
        if len(summaryData) >= 2:
            for metric in [self.config.columnAverageNumberInSystem, self.config.columnAverageWaitingTime]:
                # Use mean values for each occupancy level
                occupancyData = summaryData.groupby('occupancy')[f'{metric.split("_")[-1]}_mean'].mean()
                ...
                if len(occupancyData) >= 2:
                    ...
                    analyticalModels[metric] = {'equation': 'Insufficient data', 'rSquared': 0}
        else:
