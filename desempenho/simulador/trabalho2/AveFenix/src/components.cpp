...
Queue::Queue(int queueId, int queueCapacity, double mu, DiscardPolicy policy)
    : id(queueId), capacity(queueCapacity), serviceRate(mu), discardPolicy(policy) {
      measurementWindow.initialize(1000);
    }
