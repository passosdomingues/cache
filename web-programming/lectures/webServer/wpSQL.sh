...
FLUSH PRIVILEGES;"
        ...
        write_wp_config_safe "${WEB_ROOT_BASE}/${dbname}" "${dbname}" "${dbuser}" "${dbpass}"
    done
}

write_wp_config_safe() {
    local site_dir="$1"
    local dbname="$2"
    local dbuser="$3"
    local dbpass="$4"
    ...
    # Substitui informações do banco de dados
    perl -i -pe "
        s/'DB_NAME', '.*'/'DB_NAME', '$dbname'/;
        s/'DB_USER', '.*'/'DB_USER', '$dbuser'/;
        s/'DB_PASSWORD', '.*'/'DB_PASSWORD', '$dbpass'/;
    " "$wp_config"

    # Insere chaves do WordPress de forma segura
    local salts
    salts="$(wget -qO- https://api.wordpress.org/secret-key/1.1/salt/)"
    if [[ -n $salts ]]; then
        awk -v salts="$salts" '
            BEGIN {p=0}
            /AUTH_KEY/ {p=1; print salts; next}
            /NONCE_SALT/ {p=0; next}
            p==0 {print}
    ...
    chmod 640 "$wp_config"
    log "wp-config.php gerado com sucesso em $site_dir"
}
