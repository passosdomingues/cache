...
- Front-ends `image` e `atlas` no `FrontendRegistry` de `tools/assetc/`,
  sem alterar o núcleo do asset compiler (RFC 02, RFC 04):
  - `image_codec` — decodifica e transforma imagens via ImageMagick CLI
    (`convert`/`identify`): resize, crop, padding transparente; extrai
    RGBA8 cru; gera cadeia de mipmaps por box filter (média 2x2)
  - `atlas_packer` — shelf packing determinístico (Sprite Compiler):
    ordena sprites por altura decrescente (desempate por id), empacota
    sem sobreposição, devolve posições na ordem original de entrada
  - `compression` — deflate/zlib do payload final de `image`/`atlas`
    (dependência de Toolchain, como ImageMagick — nunca no runtime)
  - `FrontendContext` — novo parâmetro de `FrontendFn`, dá a um front-end
    acesso aos nós de IR já compilados nesta build; o `atlas` usa isso
    para ler payload/metadata das suas dependências `image`
- `AssetIRNode.metadata` e `SourceAsset.params` — metadados estruturados
  (width/height/channels/mip_count/...) e parâmetros arbitrários do
  manifesto (`resize=`, `crop=`, `pad=`, `max_width=`, `padding=`,
  `mips=`), propagados também em cache hits (persistidos em
  `BuildCache`/`CacheEntry`)
- `hash_combine` — hash do arquivo fonte combinado com hash dos
  parâmetros do manifesto, para que mudar um parâmetro invalide o cache
  mesmo sem o arquivo mudar
- Formato do pacote bump para v2 (`ASSETPK2`): tabela de metadata por
  asset; `read_package_payload` para ler o blob de um asset isolado
- `tests/assetc_tests.cpp` +4 testes: round-trip de compressão, packer
  sem sobreposição, metadata do front-end de imagem, atlas com
  verificação de pixels reais decodificados do pacote (11 no total)
- `tools/assetc/examples/atlas.manifest` + PNGs de exemplo
- Novas dependências de Toolchain documentadas no `README.md`:
  ImageMagick e zlib (`tools/assetc/CMakeLists.txt` usa
  `find_package(ZLIB REQUIRED)`)
- `Makefile`: atalho `make atlas-example`
