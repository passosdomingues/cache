...
namespace {
// Hash de conteudo do arquivo fonte combinado com um hash dos parametros
// do manifesto (resize=, crop=, max_width=, ...) — mudar um parametro
// invalida o cache mesmo que o arquivo fonte nao tenha mudado.
std::uint64_t compute_source_hash(const SourceAsset& source) {
    std::uint64_t file_hash = source.source_path.empty() ? 0 : hash_file(source.source_path);

    std::string params_repr;
    for (const auto& [key, value] : source.params) {
...
BuildStats build(const BuildOptions& options) {
    ...
    // Nós já compilados nesta build, para front-ends de agregação (ex.:
    // atlas) lerem via FrontendContext. Ponteiros permanecem válidos
    // porque `nodes` tem capacidade reservada para o total de assets.
    std::unordered_map<std::string, const AssetIRNode*> compiled_by_id;

    // Ids recompilados nesta rodada — propaga invalidação de cache pelo
    // grafo de dependências: se uma dependência foi recompilada, quem
    // depende dela também é, mesmo que o próprio arquivo fonte não tenha
    // mudado (necessário para front-ends cujo resultado depende do
    // conteúdo das dependências, ex.: atlas).
    ...
    for (const auto& source : ordered) {
        ...
        std::uint64_t source_hash = compute_source_hash(source);
        std::string frontend_tag = source.type + "@" + std::to_string(frontend->version);

        bool deps_dirty = std::any_of(source.dependencies.begin(), source.dependencies.end(),
                                       [&](const std::string& dep_id) { return dirty_ids.count(dep_id) > 0; });
        ...
        if (!cache_hit) {
            FrontendContext context{compiled_by_id};
            node = frontend->compile(source, context);
            node.content_hash = source_hash;
            ...
            cache.put(source.id, CacheEntry{
                ...
                frontend_tag, source.dependencies, output_hash, node.metadata,
            });
