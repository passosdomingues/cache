...
// Codificação interna e confiável (não exposta ao usuário) para a lista
// de metadados dentro de um único campo do manifesto de cache.
std::string encode_metadata(const std::vector<std::pair<std::string, std::string>>& metadata) {
    std::string out;
...
std::vector<std::pair<std::string, std::string>> decode_metadata(const std::string& value) {
...
void BuildCache::load() {
    ...
    for (const auto& block : parse_blocks(*content)) {
        ...
        entry.metadata = decode_metadata(block.get("metadata"));
...
void BuildCache::save() const {
    ...
    for (const auto& id : ids) {
        ...
        block.fields = {
            ...
            {"metadata", encode_metadata(entry.metadata)},
        };
