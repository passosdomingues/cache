...
void write_package(const std::filesystem::path& out_path, const std::vector<PackageEntry>& entries) {
    ...
    for (std::size_t i = 0; i < entries.size(); ++i) {
        ...
        running_offset += entries[i].payload.size();
    }

    for (std::size_t i = 0; i < entries.size(); ++i) {
        const auto& entry = entries[i];
        write_string(out, entry.id);
        write_string(out, entry.type);
        write_string(out, entry.source_path);
        write_u64(out, entry.content_hash);

        write_u32(out, static_cast<std::uint32_t>(entry.dependencies.size()));
        for (const auto& dep_id : entry.dependencies) {
            auto it = index_by_id.find(dep_id);
            if (it == index_by_id.end()) {
                throw std::runtime_error("dependencia desconhecida ao empacotar '" + entry.id + "': " + dep_id);
            }
            write_u32(out, it->second);
        }

        write_u32(out, static_cast<std::uint32_t>(entry.metadata.size()));
        ...
        write_u64(out, static_cast<std::uint64_t>(entry.payload.size()));
    }
